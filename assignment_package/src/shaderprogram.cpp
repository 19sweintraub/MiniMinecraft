...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1), attrUV(-1), attrAnimCos(-1),
...
void ShaderProgram::setupMemberVars() {
    ...
    attrAnimCos = context->glGetAttribLocation(prog, "vs_AnimCos");
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if(d.bindOpInter()) {
        if (attrPos != -1) {
            ...
            context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, size, 0);
        }

        if (attrNor != -1) {
            context->glEnableVertexAttribArray(attrNor);
            context->glVertexAttribPointer(attrNor, 4, GL_FLOAT, false, size, (GLvoid*)(sizeof(glm::vec4)));
        }

        if (attrCol != -1) {
            context->glEnableVertexAttribArray(attrCol);
            context->glVertexAttribPointer(attrCol, 4, GL_FLOAT, false, size, (GLvoid*)(2*sizeof(glm::vec4)));
        }

        if (attrUV != -1) {
            context->glEnableVertexAttribArray(attrUV);
            context->glVertexAttribPointer(attrUV, 2, GL_FLOAT, false, size, (GLvoid*)(3*sizeof(glm::vec4)));
        }

        if (attrAnimCos != -1) {
            context->glEnableVertexAttribArray(attrAnimCos);
            context->glVertexAttribPointer(attrAnimCos, 2, GL_FLOAT, false, size, (GLvoid*)(3*sizeof(glm::vec4)+sizeof(glm::vec2)));
        }

        // Bind the index buffer and then draw shapes from it.
        // This invokes the shader program, which accesses the vertex buffers.
        d.bindOpIdx();
        context->glDrawElements(d.drawMode(), d.elemCountOp(), GL_UNSIGNED_INT, 0);

        if (attrPos != -1) context->glDisableVertexAttribArray(attrPos);
        if (attrNor != -1) context->glDisableVertexAttribArray(attrNor);
        if (attrCol != -1) context->glDisableVertexAttribArray(attrCol);
        if (attrUV != -1) context->glDisableVertexAttribArray(attrUV);
        if (attrAnimCos != -1) context->glDisableVertexAttribArray(attrAnimCos);
    }
}
...
void ShaderProgram::draw(Drawable &d, int textureSlot = 0)
{
    ...
    if(d.bindTrInter()) {
        ...
        // This invokes the shader program, which accesses the vertex buffers.
        d.bindTrIdx();
        context->glDrawElements(d.drawMode(), d.elemCountTr(), GL_UNSIGNED_INT, 0);
